...
class Autocomplete extends Component {
  ...
  onChange = e => {
    ...
    if (lastChar === " ") {
      ...
      axios.post("http://localhost:3001/autocorrect", data).then(response => {
  ...
  render() {
    ...
    if (this.state.a) {
      redirectVar = (
        <Redirect
          to={{ pathname: "/article", state: { referrer: this.state.title } }}
        />
      );
    }
      ...
      return (
        <tr onClick={() => this.expandArticle(article._source.title)}>
          <td>
            ...
            <h1>
              - {article._source.author}, {article._source.date}
            </h1>{" "}
            {<div> {article._source.summary} </div>}
          </td>
        </tr>
      );
    });
        ...
        <div class="top-head left">
          <div class="container">
            <div class="row">
              <h1 align="center">
                {" "}
                San Jose Times <small>Get the latest News</small>
              </h1>
            </div>
